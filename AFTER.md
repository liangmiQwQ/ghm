...
[Eval] Ensure whether these shell exist, or exit with code 69. Prompt users: "Would you like to add alias for ghm use?"

[Action A] User entry `Y`
[Eval A] Prompt user: "Which alias would you like to use?" for each command. One command can have multiple aliases with `,` as separator. Allow to leave blank (no alias) and provide default aliases as well.

[Action B] User entry `N`
[Eval B] Jump to Eval C

[Eval C] Generate the config file, and write shellrc. Load `ghm shell` with `source` cmd as well.
